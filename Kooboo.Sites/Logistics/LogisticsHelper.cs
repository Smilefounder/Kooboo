...
namespace Kooboo.Sites.Logistics
{
    ...
    {
        public static string GetCallbackUrl(ILogisticsMethod logisticsMethod, string MethodName, RenderContext context)
        {
            var baseurl = GetBaseUrl(context).TrimEnd('/');
            return baseurl + "/_api/logiticscallback/" + logisticsMethod.Name + "_" + MethodName;
        }
